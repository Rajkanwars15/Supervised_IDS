"""
Decision Tree Experiment for NSL-KDD Binary Classification Dataset

This script trains a decision tree classifier on the NSL-KDD dataset and logs:
...
if not train_path.exists() or not test_path.exists():
    ...
    print(f"  python experiments/nsl-kdd/scripts/preprocess_nsl_kdd.py")
    sys.exit(1)
...
try:
    if plt is not None:
        ...
        plot_tree(clf, 
                 ...
                 ax=ax)
        plt.title(f'Decision Tree Visualization (max_depth=5) - NSL-KDD', fontsize=16)
        plt.tight_layout()
...
with open(tree_text_file, 'w') as f:
    f.write(f"Decision Tree Structure - NSL-KDD Experiment {timestamp}\n")
    f.write("="*80 + "\n\n")
    f.write(tree_rules)
