"""
Decision Tree Experiment for CIC IOV 2024 Binary Classification Dataset

This script trains a decision tree classifier on the CIC IOV 2024 dataset and logs:
...
# Set paths
data_dir = Path(__file__).parent.parent.parent.parent / "data" / "cic-iov-2024-silver"
train_path = data_dir / "CIC-IOV-2024_Train_Binary.csv"
test_path = data_dir / "CIC-IOV-2024_Test_Binary.csv"

# Check if silver data exists
if not train_path.exists() or not test_path.exists():
    print("ERROR: Preprocessed data not found!")
    print(f"Please run preprocessing script first:")
    print(f"  python experiments/cic-iov-2024/scripts/preprocess_cic_iov_2024.py")
    sys.exit(1)
...
try:
    if plt is not None:
        ...
        plot_tree(clf, 
                 ...
                 ax=ax)
        plt.title(f'Decision Tree Visualization (max_depth=5) - CIC IOV 2024', fontsize=16)
        plt.tight_layout()
...
with open(tree_text_file, 'w') as f:
    f.write(f"Decision Tree Structure - CIC IOV 2024 Experiment {timestamp}\n")
    f.write("="*80 + "\n\n")
    f.write(tree_rules)
...
results = {
    ...
    'dataset': 'CIC IOV 2024 Binary Classification',
