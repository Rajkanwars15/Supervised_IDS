"""
Decision Tree Experiment for UNSW-NB15 Binary Classification Dataset

This script trains a decision tree classifier on the UNSW-NB15 dataset and logs:
...
# Set paths
data_dir = Path(__file__).parent.parent.parent.parent / "data" / "unsw-nb15-silver"
train_path = data_dir / "UNSW-NB15_Train_Binary.csv"
test_path = data_dir / "UNSW-NB15_Test_Binary.csv"

# Check if silver data exists
if not train_path.exists() or not test_path.exists():
    print("ERROR: Preprocessed data not found!")
    print(f"Please run preprocessing script first:")
    print(f"  python experiments/unsw-nb15/scripts/preprocess_unsw_nb15.py")
    sys.exit(1)
...
try:
    if plt is not None:
        ...
        plot_tree(clf, 
                 ...
                 ax=ax)
        plt.title(f'Decision Tree Visualization (max_depth=5) - UNSW-NB15', fontsize=16)
        plt.tight_layout()
...
with open(tree_text_file, 'w') as f:
    f.write(f"Decision Tree Structure - UNSW-NB15 Experiment {timestamp}\n")
    f.write("="*80 + "\n\n")
    f.write(tree_rules)
